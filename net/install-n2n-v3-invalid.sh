...
# 另外还支持用户名\密码的验证形式
demo1
# 测试账号：Z6jrsE2u  测试密码：R0npLVSuBC%ey5M
# $N2N_BIN_DIR/n2n-keygen Z6jrsE2u R0npLVSuBC%ey5M
demo2
* Z6jrsE2u VKWEJ+-eiepITx+xoJa0ioBremZpkI-DBAi1RQul7Wq
...
# -c                 # 组名称 | 用于区分虚拟局域网
# -I                 # 用户名 | 用于需要用户名密码认证的supernode
# -J                 # 密码 | 用于需要用户名密码认证的supernode
# -A?                # 加密方式 | 比如-A4为ChaCha20，-A5为SPECK
# -k                 # -A?对应的密钥 | 用于虚拟局域网内传输的数据加密，留空则不加密
# -f                 # 前台运行（systemd 管理时需要加上 -f）
...
# 当使用了“-I”和“-J”之后，报头加密已经自动启用，因为此身份验证方案严重依赖它。
# 目前只有流密码在安全性方面可靠地与此身份验证方案一起工作。因此，需要-A4 ChaCha20或-A5 SPECK以及密钥-k <key>作为附加参数。

# 主命令
$N2N_BIN_DIR/edge -l n2n.demo.com:49527 -c demo2 -I Z6jrsE2u -J "R0npLVSuBC%ey5M" -A5 -k "demokey1&23" -f
...
# 参考命令
$N2N_DIR/edge -l n2n.demo.com:49527 -c demo1 -k "demokey1&23"
